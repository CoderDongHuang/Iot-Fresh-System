...
export function sendControlCommand(vid: string, command: any): Promise<ResponseData> {
  return service.post(`http://localhost:8080/api/control/command/${vid}`, command)
}

// 获取控制历史
export function getControlHistory(vid: string, params: any): Promise<any> {
  return service.get(`http://localhost:8080/api/control/history/${vid}`, { params })
}

// 获取设备控制参数
export function getDeviceControlParams(vid: string): Promise<any> {
  return service.get(`http://localhost:8080/api/control/params/${vid}`)
}

// 保存控制参数
export function saveControlParams(vid: string, params: any): Promise<ResponseData> {
  return service.post(`http://localhost:8080/api/control/params/${vid}`, params)
}